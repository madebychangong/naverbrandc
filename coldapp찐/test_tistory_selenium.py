...
def test_tistory_posting():
    ...
    try:
        ...
        # 3. 테스트 데이터 준비
        title = "테스트 글 - 티스토리 Selenium 자동 포스팅"

        ai_result = {
            'content': """
[TEXT] 티스토리 Selenium 자동 포스팅 테스트입니다.

[QUOTE:UNDERLINE] 주요 기능

[TEXT] 이 글은 Selenium을 이용한 자동 포스팅 테스트입니다.

[QUOTE:VERTICAL] HTML 스타일이 제대로 적용되는지 확인합니다.

[TEXT] 이미지와 링크도 정상적으로 삽입되는지 테스트합니다.

[IMAGE:1]

[LINK]
""",
            'tags': ['테스트', '자동화', 'Selenium', '티스토리']
        }

        # 테스트용 이미지 (실제 경로로 변경하세요)
        image_files = [
            "temp_images/test_image.jpg"  # 실제 존재하는 이미지 경로
        ]
